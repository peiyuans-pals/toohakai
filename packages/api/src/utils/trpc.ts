...
// }
  ...
  const session = null // opts?.auth ?? null // ?? (await auth()); // todo
  const source =  "unknown" // opts?.req?.headers?.get ? opts?.req?.headers?.get("x-trpc-source") : "unknown";

  log(`>>> tRPC Request from ${source} by`); // ${session?.user}`);
